...
export function StepByStepSolution({ steps, taskId, open, onClose }: Props) {
                ...
                {revealedCount > 1 && (
                  <>
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 text-sm text-gray-500 hover:text-gray-700 py-2 rounded-lg hover:bg-white transition-colors border border-transparent hover:border-gray-200"
                    >
                      <Code2 size={14} />
                      {showFullCode ? 'Скрыть полный код' : 'Показать полный код'}
                      {showFullCode ? (
                        <ChevronUp size={14} />
                      ) : (
                        <ChevronDown size={14} />
                    ...
                    <AnimatePresence>
                      {showFullCode && (
                        <motion.div
                          ...
                          className="overflow-hidden"
                        >
                          <div className="bg-[#1e1e2e] rounded-xl overflow-hidden mb-3 shadow-lg">
                            <div className="flex items-center justify-between px-4 py-2 bg-[#181825]">
                              <span className="text-[11px] text-gray-500 font-mono">
                                Полный код — {revealedCount} из {steps.length} шагов
                              </span>
                            </div>
                            <pre className="p-4 overflow-x-auto max-h-[240px]">
                              <code className="text-sm font-mono leading-relaxed text-[#cdd6f4] whitespace-pre">
                                {highlightPython(accumulatedCode)}
                              </code>
                            </pre>
                          </div>
                        </motion.div>
